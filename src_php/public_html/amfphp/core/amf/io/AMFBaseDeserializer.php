...
require_once(AMFPHP_BASE . "../globals.php");
...
class AMFBaseDeserializer {
    ...
    function mapClass($typeIdentifier)
    {
		...
		$include = FALSE;
		if(file_exists("$voPath/$mappedClass"))
		{
			$include = "$voPath/$mappedClass";
		}
		elseif(file_exists($GLOBALS['amfphp']['customMappingsPath'] . $mappedClass . '.php'))
		{
