		...
		if( $classDefinition['externalizable'] )
		{
			...
			else
			{
				...
				if($package == FRONTEND_MODELS_PACKAGE)
				{
					...
					$valueObject->merge($obj, false);
					
					$relations = $valueObject->getTable()->getRelations();
					foreach($relations as $key => $val)
					{
						$valueObject->clearRelated($key);
						...
						if($val["type"] == Doctrine_Relation::MANY)
						{
							...
							$valueObject->$key = $obj[$key];
						//$valueObject->images[0] = $obj["images"][0];
					}
					
//					$relations = $valueObject->getTable()->getRelations();
//					foreach($relations as $key => $val)
//					{
//						if(is_array($obj[$key]))		// if related data is an array, convert it to a Doctrine_Collection
//						{
//							$valueObject->loadReference($key);
//							
//							$ref = $valueObject->getTable()->getRelation($key);
//							$relatedClass = $valueObject->getTable()->getRelation($key)->getClass();
//							$collection = new Doctrine_Collection($relatedClass);
//							foreach($obj[$key] as $related)
//							{
//								NetDebug::trace($key);
//								$collection->add($related);
//							}
//							
//							$for = new $relatedClass();
//							NetDebug::printr($ref->getLocalFieldName().":".$for->getRelation("user"));
//								
//							$valueObject->setRelated($key, $collection);
//						}
//						else if(!empty($obj[$key]))
//							$valueObject->$key = $obj[$key];
//					}
